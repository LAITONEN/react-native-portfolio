...
import { adjust } from '../components/reusable/Library';
...
export const loadExplanationDictionary = (navigation, dictionary) => {
	...
	return (dispatch) => {
		DATABASE(access).on('value', snapshot => {
			dispatch({ type, dictionary, payload: snapshot.val() });
		});
	};
};

// load Native Language to English dictionary and then on every change in the dictionary object
export const loadTranslationDictionary = (navigation, dictionary) => {
...
export const loadVarietyDictionary = (navigation, dictionary) => {
...
export const filterList = (list, navigation, predicate) => {
	const { dictionary } = navigation.state.params;
	const object = { type: 'filter_list', list, dictionary, predicate };
...
// DECLARATIONS (grouped and sorted in the use order)
	...
	const { dictionary } = params; // prop that hold string of the dictionary name to add a word to
	const error = () => definitions === '' || term.length < 2;
	const payload = 'Fill in the first two fields!';
	const type = EMPTY_FIELDS_ERROR;

	const access = `/users/${firebase.auth().currentUser.uid}/${dictionary}`; 
...
// DECLARATIONS (grouped and sorted by in the by use order)
	...
	const access = `/users/${firebase.auth().currentUser.uid}/${dictionary}/${uid}`;
	const word = { definitions, examples, rules, synonyms, tags, term: term.trim() };

	const { params } = navigation.state;
	const details = { definitions, examples, dictionary, rules, uid, synonyms, tags, term };
	const paramsForDetails = { ...params, word: details };
	// reset Action that resets navigation state
	// and navigates to the WordDetails page stacked on top of the list page
	// list page receives generic params, 
	// details page, in addition receives word object
	const resetAction = NavigationActions.reset({ index: 1, actions: [
...
// DECLARATIONS
	...
	const { dictionary, word } = params;

	const access = `/users/${currentUser.uid}/${dictionary}/${word.uid}`;
	const actions = [NavigationActions.navigate({ routeName, params })];
	const resetToList = NavigationActions.reset({ index: 0, actions });
